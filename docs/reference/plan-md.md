...
| `gap_closure` | Only in gap-closure mode | boolean | `true` on every plan generated by `/gsd:plan-phase --gaps`. `/gsd:execute-phase --gaps-only` filters strictly on this field, so an omitted `gap_closure` on a gap-closure plan means it is silently skipped — zero executors spawned, no error (#2847). Standard and reviews-mode plans must never carry it; the planner validates gap-closure plans against the `plan-gap-closure` schema (this field plus every field above) instead of `plan`. |
