...
| `coupling_justified` | No | array | `["<other-plan-id>", "<reason>"]` — declares that a same-wave coupling with the named plan through a shared mutable resource (config key, table, migration, env var) is deliberate and order-independent. The plan-checker's Dimension 3b recognizes the declaration and does not flag the pair, so intentionally coupled plans can pass verification without serializing waves (#3724). |
