...
| effortSurface | undocumented | searched the runtime's official docs (see Sources consulted above) | No reasoning-effort setting is documented for this host, so the axis carries the fail-closed sentinel rather than inheriting a profile baseline (#2481). |
