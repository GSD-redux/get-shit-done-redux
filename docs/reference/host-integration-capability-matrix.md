...
- **Hook events** — GSD registers the supported `SessionStart` update check through `hooks.json`. Codex does not expose the remaining-context metric consumed by `gsd-context-monitor.js`, so fresh installs do not copy or register that monitor. Reinstall removes only exact current or recognized legacy GSD-owned monitor registrations; customized and unknown commands are preserved. The descriptor therefore declares no extended hook events, `agentFacingContextWarnings: false`, and `gsdPhaseLifecycleDisplay: false`.
