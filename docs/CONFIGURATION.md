...
| `hooks.context_warning_threshold` | number | `35` | Percent of context window REMAINING at or below which the monitor emits CONTEXT WARNING. Must be 0-100 and strictly greater than `hooks.context_critical_threshold`; an out-of-domain or inconsistent pair falls back to both defaults rather than firing at a nonsense point (#4285) |
| `hooks.context_critical_threshold` | number | `25` | Percent of context window REMAINING at or below which the monitor escalates to CONTEXT CRITICAL. Must be 0-100 and strictly less than `hooks.context_warning_threshold`. Setting only one of the pair is checked against the other's default, so tune both when moving either past the other (#4285) |
