...
| `task-command-router.cjs` | Thin CJS subcommand router adapter for `gsd-tools task`; `resolve-content` subcommand resolves a task's `tracker-id` via `task-content-resolution.cjs` (ADR-3646, #3970); `red-evidence-verdict` subcommand dispatches `red-evidence-predicate.cjs` and additionally requires `--changed-files` to include the trailer's `location.declared.file` before an otherwise-authorizing verdict stands (D-1, #3770 — see that module's row below) |
