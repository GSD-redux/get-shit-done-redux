...
| `test-home-guard.cjs` | Test-home confinement guard (#3712) — refuses an in-process `installRuntimeArtifacts` / `uninstallRuntimeArtifacts` / `applySurface` call (plus the descriptor-dependent `installOpencodeFamilySkills` and `installAgentsKindStandalone`) when a `node --test` run would resolve a kind's global `home` override (codex skills -> `$HOME/.agents`, ADR-1239/#2088) inside the real passwd home, which silently pruned every `gsd-*` skill there; compares homes by filesystem identity (`st_dev`+`st_ino`) rather than by pathname, since a case-variant, symlinked, or bind-mounted HOME names one directory under two names; fails CLOSED unless both homes identify or one is definitively absent, with `sandboxHome()`'s path-valued marker as the sandboxed-caller escape where no passwd entry is readable; inert for normal installs outside a Node test context |
