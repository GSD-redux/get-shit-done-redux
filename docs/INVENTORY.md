...
| `debug-session-policy.cjs` | Pure fail-closed parser for a saved debug session's optional Runtime Evidence policy; rejects ambiguous sections, comments, fences, malformed records, invalid UTF-8/NUL input, and unsupported values before `init.debug` resolves continuation policy (#3128) |
