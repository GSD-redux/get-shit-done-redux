...
| `red-evidence-predicate.cjs` | RED-evidence predicate for the MVP+TDD gate (compiled from `src/red-evidence-predicate.cts`, gitignored; #3770) — leaf-pure evaluator (no fs, no child_process, no config) implementing tdd.md's `### RED Predicate` fence: exact eight-key trailer set equality, `location.declared`/`location.observed` shape and agreement (`path.win32.basename` separator split, strict line, never a column), `trailer.expected == plan.expected_failure`, `trailer.target_test == plan.target_test`, and the two-arm disjunction (target-test-selected-and-executed, or outside-in with a missing declared target). Returns the three-valued verdict `authorize` / `red_commit_not_failing` / `unexpected_pass` with a bounded `reason` naming the deciding conjunct; fails closed on an absent, malformed, or over-shaped record. Dispatched behind `task-command-router.cjs`'s `red-evidence-verdict` subcommand, which the gate block in `gsd-core/workflows/execute-phase.md` consults before authorizing GREEN |
