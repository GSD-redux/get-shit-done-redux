...
- **Amended by:** [ADR-2363](2363-capability-instruction-surface-trust.md) (Capability instruction-surface trust) — **D5 gains a fifth disclosure class**, the instruction surface (skills, agents), which is the first disclosed class that is *not* an executable surface at all: it is disclosed by name at install, but it never sets `hasExecutable` and never enters the `disclosureSignature`, so it carries no consent or re-consent weight of its own.
