...
- **Out-of-band, where the return is a bare value that cannot carry provenance.** A read that returns a bare sentinel or a plausible default keeps returning exactly that, and emits a **deduplicated `stderr` diagnostic** naming the file and the errno. The repo already has this seam — `config-loader.cts`'s `_warnedUnknownConfigKeys` guard around `process.stderr.write` — and it is what ADR-227 means by *"an opt-in debug warn preserves visibility."* This covers `getRoadmapPhaseInternal` (#1881), `findContextMdIn` / `listMilestoneArchiveDirs` (#1883), and `getMilestoneInfo` (#1881), whose fallback is a populated `{ version: 'v1.0', name: 'milestone' }` rather than an empty sentinel — a plausible-looking default is *more* in need of a diagnostic than an empty one, not less.
...
**Structural checks are the author's own work.** Where there is no exception to surface (#1882: `extractFrontmatter` takes a `string`, does no I/O, and has no throw site), "make it visible" means the function must *detect* the malformed shape and distinguish it from the well-formed-and-empty shape. There is nothing to propagate; the check has to be written.
...
**Caller audit is mandatory per applier.** Each applying PR audits its callers and records the result, even though these mechanisms are designed to be non-breaking — the point is to prove it, not assume it. `src/state.cts` carries a comment recording that a defensive `try/catch` around `getMilestoneInfo` was **deliberately removed** under the #2245 audit because that function "never throws"; that invariant is exactly the kind a careless fix breaks, and exactly why the audit is not optional.
...
First appliers: #1880 (in-band), #1881 / #1883 (out-of-band), #1882 (structural check), #1884 (genuinely-fatal carve-out, already throwing) — epic #1879, Phase 0 = #2674.
