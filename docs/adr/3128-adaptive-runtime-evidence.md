...
| Fact | `src/init.cts` — `cmdInitDebug` folds explicit flag → valid saved session policy → `off` default, then emits `runtime_evidence_eligible = (policy !== 'off' || valid continue route)` so off-policy cleanup cannot be gated away |
