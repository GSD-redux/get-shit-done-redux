...
This mirrors the `Supersedes`/`Superseded by` and `Subsumes`/`Subsumed by` reciprocity rule in [`docs/adr/README.md`](adr/README.md#3-supersession-and-subsumption-are-symmetric) — a one-way `Amends` pointer is the same failure mode: a reader who lands on the original ADR has no way to know it was extended. See `857-capability-system.md`, amended by `1244-capability-ecosystem.md` (extends D7/D8) and `2782-reviewer-lane-capability-surface.md` (extends "extension points as data") — both record the back-link on 857 in the same PR that added the `Amends` field.
