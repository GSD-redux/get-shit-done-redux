...
- **Antigravity:** `~/.gemini/config/` (default) or auto-detected legacy/IDE roots (`~/.gemini/antigravity/`, `~/.gemini/antigravity-ide/`, or `~/.gemini/antigravity-cli/`) or `./.agents/` (or legacy `./.agent/`) local
...
| Antigravity | `~/.gemini/config` (default; auto-detected: `~/.gemini/antigravity`, `~/.gemini/antigravity-ide`, or `~/.gemini/antigravity-cli`) | `./.agents` (legacy `./.agent` detected) | `skills/gsd-*/SKILL.md` (flat, #1614) | `agents/gsd-*.md` | Gemini-style `settings.json` hook entries when installed by GSD |
