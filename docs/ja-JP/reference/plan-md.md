...
| `files_deleted` | いいえ | パスの配列 | このプランが意図的に**削除**するファイル。ウェーブ後のクリーンアップ検査は、差分にファイル削除を含むエグゼキューターブランチのマージを一律にブロックします（大量削除事故に対する安全網）。このフィールドは、その例外を明示するオプトインです。照合は区切り文字を正規化したうえでのパス完全一致です。宣言されたパスはマージされ、宣言されていない削除は依然としてそのプランのエントリのみをブロックします。グロブもディレクトリ接頭辞もないため、宣言が文字どおり列挙した以上の権限を得ることは決してありません。フィールドを省略すればガードの元の無条件ブロックがそのまま有効なので、省略が常に安全側の既定です（#3003）。 |
