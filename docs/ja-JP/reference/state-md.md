...
**このセクションのすべてのフィールドは単一値であり、このセクションは追記されるのではなく上書きされます。** 2つ目の `Phase:` 行は2つ目の位置ではなく、履歴でもありません — それは不正な入力です。リーダーは重複を文書内の出現順序では解決しません。**フォーム**によって解決します。判定順序は各フォームが文書内のどこに現れるかに関係なく、太字の `**Phase:** value`（文書内のどこにあっても）、次に行頭の平文 `Phase: value`、最後にパイプテーブルの `| Phase | value |` 行の順です — そして勝者となったフォーム**内**でのみ、**最初の**出現が勝ちます。実務上の帰結は2つあります。上位のフォームで書かれた重複は、ファイル内で先に現れていても勝ちます。つまり「強調のために」追記された太字行は、無視されるのではなく、先行する平文行を黙って上書きします。また、インデントされた `Phase:` 行は平文フォーム（行頭を基準とする）からは見えず、次に一致するフォームへとフォールスルーします。このセクションを書き込む際は、行を追加するのではなく、置き換えてください。
