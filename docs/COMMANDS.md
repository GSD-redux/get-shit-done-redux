...
> **Untrustworthy-window guard.** Archiving also refuses when the milestone's ROADMAP window could not be fully resolved — `Cannot mark milestone complete: the ROADMAP window for "<version>" is truncated|unscoped|unreadable`. This is the case where the milestone's heading is found but its section closes before reaching the roadmap's `### Phase N:` region (typically a closed-milestone heading sitting in between), which previously degraded to an over-inclusive filter and archived *every* phase directory in the project rather than the milestone's own. Same override as below: `gsd-tools milestone complete <version> --force`. A window that is genuinely empty — a freshly-declared milestone with no phases yet — is *not* affected and still completes normally.
