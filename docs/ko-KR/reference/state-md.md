...
**이 섹션의 모든 필드는 단일 값이며, 이 섹션은 추가되는 것이 아니라 덮어써집니다.** 두 번째 `Phase:` 줄은 두 번째 위치가 아니며 이력도 아닙니다 — 이는 잘못된 형식의 입력입니다. 리더는 중복을 문서 내 등장 순서로 해석하지 않습니다. **형식(form)**에 따라 해석합니다. 판정 순서는 각 형식이 문서 어디에 나타나든 관계없이, 문서 내 어디에든 있는 굵은 글씨 `**Phase:** value`, 그다음 줄 시작에 오는 일반 텍스트 `Phase: value`, 마지막으로 파이프 테이블의 `| Phase | value |` 행 순입니다 — 그리고 승리한 형식 **내에서만** **첫 번째** 항목이 우선합니다. 실무적으로 두 가지 결과가 따릅니다. 상위 형식으로 작성된 중복은 파일 내에서 먼저 등장하더라도 승리합니다. 즉 "강조를 위해" 뒤에 추가된 굵은 글씨 줄은 무시되는 대신 앞선 일반 텍스트 줄을 조용히 덮어씁니다. 또한 들여쓰기된 `Phase:` 줄은 (줄 시작을 기준으로 삼는) 일반 텍스트 형식에는 보이지 않으며, 다음으로 일치하는 형식으로 넘어갑니다. 이 섹션을 작성할 때는 줄을 추가하지 말고 교체하세요.
