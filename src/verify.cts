...
// #612: `phaseDirNameRe` / `PHASE_TOKEN_FROM_DIR_RE` are replaced by the gated
// `isPhaseDirName` / `phaseTokenFromDir` helpers (a convention-less call
// delegates to the same two regexes, so a non-bracket repo is byte-identical).
// `canonicalPlanStem` left with #3183: its sole consumer here — the bespoke
// I001 PLAN/SUMMARY pairing — now routes through `findUnsummarizedPlans`.
import { MILESTONE_ARCHIVE_DIR_RE, isPhaseDirName, phaseTokenFromDir, textEncodingError } from './validate.cjs';
