        ...
        if (!cached) {
          ...
          if (roadmapScope !== null) {
            ...
            while ((m = phaseHeadingPattern.exec(roadmapScope)) !== null) {
              ...
              // #3185 owns the legacy token predicate. The bracket leg keeps
              // #612's narrower bare-token rules: `00.1` is a real bracket phase,
              // while the canonical leading-integer predicate classifies it as
              // milestone-0 sentinel. The bracket-qualified predicate above
              // remains the owner of sentinel milestone ids.
              if (phaseConvention === 'bracket') {
                if (/^0\b/.test(token) || /^999\b/.test(token)) continue;
              } else if (isSentinelPhaseId(token)) {
                continue;
              }
