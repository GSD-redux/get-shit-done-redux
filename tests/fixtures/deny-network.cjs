 ...
 * `connect`, `dns` lookup/resolve variants, and `global(This).fetch` (when
 * defined) — with a function that throws synchronously, so ANY attempt to
 * use one of these surfaces fails loudly and immediately rather than hanging
 * or silently succeeding against a real or mocked network.
 *
