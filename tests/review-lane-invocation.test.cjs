...
const GOLDEN = [
  ...
  { slug: 'coderabbit', binary: 'coderabbit', argv: ['review', '--prompt-only'], stdin: false, out: 'stdout', timeout: 360000 },
  {
    slug: 'opencode',
    binary: 'opencode',
    argv: ['run', '--pure', '--agent', 'plan', '--model', 'O', '--effort', 'high', '--format', 'json', '--print-logs', '--log-level', 'ERROR', '-'],
    stdin: true, out: 'stdout', timeout: 660000,
  },
  {
    slug: 'zcode',
    binary: 'zcode',
    ...
    stdin: false, out: 'stdout', timeout: 900000,
  },
