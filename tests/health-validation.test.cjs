...
// ── W025 (#2572): phase-SUMMARY artifact↔git advisory ────────────────────────
//
// Runs the existing verify-summary artifact check against each phase SUMMARY.
// Advisory ONLY: findings append to warnings[] and never touch `status` or the
// hard completion gate (readVerificationStatus).
//
// MECHANISM DEVIATION from the approved brief, recorded here and on #2572:
// the brief named cmdStateCompletePhase as carrying "the existing warnings[]
// pre-scan". It does not — that handler emits { updated, phase } with no
// warnings channel. The nearest state-side channel (cmdStateValidate) cannot
// consume verifySummaryCore either, because verify.cts already imports
// state.cjs and the reverse edge would be this codebase's first circular
// import (CLAUDE.md: "Circular imports: None known"). validate.health is the
// advisory diagnostic that already owns a coded warnings[] channel and lives
// in the same module as the core — zero new import edges.
describe('W025 — phase SUMMARY artifact check (#2572)', () => {
  ...
  function project(summaries, { git = true } = {}) {
    const base = fs.mkdtempSync(path.join(os.tmpdir(), 'gsd-2572-h-'));
    dirs.push(base);
    const planningDir = path.join(base, '.planning');
    const phaseDir = path.join(planningDir, 'phases', '01-one');
    fs.mkdirSync(phaseDir, { recursive: true });
    fs.writeFileSync(
      path.join(planningDir, 'PROJECT.md'),
      '# Project\n\n## What This Is\nTest.\n\n## Core Value\nTest.\n\n## Requirements\nTest.\n',
    ...
    fs.writeFileSync(
      path.join(planningDir, 'STATE.md'),
      ['# State', '', '**Current Phase:** 1', '**Status:** In progress', ''].join('\n'),
    );
  ...
  const w025 = (d) => (d.warnings ?? []).filter((w) => w.code === 'W025');

  test('warns when a phase SUMMARY references a file that does not exist', () => {
    const data = health(project({ '01-01-SUMMARY.md': '# Summary\n\nCreated: `src/ghost.ts`\n' }));
    const hits = w025(data);
    assert.strictEqual(hits.length, 1, `expected one W025, got ${JSON.stringify(data.warnings)}`);
    assert.ok(/01-01-SUMMARY\.md/.test(hits[0].message), `must name the summary: ${hits[0].message}`);
    assert.ok(/ghost\.ts/.test(hits[0].message), `must name the missing file: ${hits[0].message}`);
  });

  test('stays silent when every referenced file exists', () => {
    const data = health(project({ '01-01-SUMMARY.md': '# Summary\n\nCreated: `01-01-PLAN.md`\n' }));
    assert.strictEqual(w025(data).length, 0, `expected silence, got ${JSON.stringify(w025(data))}`);
  });

  test('checks EVERY summary in a multi-plan phase, not just the first', () => {
    // Open question (b), resolved: a phase carries N per-plan summaries plus a
    // legacy bare SUMMARY.md. The repo-wide filter is
    // `endsWith('-SUMMARY.md') || === 'SUMMARY.md'` — all of them get checked.
    const data = health(project({
      '01-01-SUMMARY.md': '# Summary\n\nCreated: `01-01-PLAN.md`\n',
      '01-02-SUMMARY.md': '# Summary\n\nCreated: `src/second-ghost.ts`\n',
      'SUMMARY.md': '# Summary\n\nCreated: `src/legacy-ghost.ts`\n',
    }));
    const msgs = w025(data).map((w) => w.message).join(' | ');
    assert.ok(/01-02-SUMMARY\.md/.test(msgs), `second plan's summary must be checked: ${msgs}`);
    assert.ok(/legacy-ghost\.ts/.test(msgs), `bare SUMMARY.md must be checked too: ${msgs}`);
  });

  test('is advisory only — lands in warnings[], never errors[] or the repair set', () => {
    // Deliberately NOT "dirty.status === clean.status". Health derives status
    // from warnings BY DESIGN (any warning -> `degraded`), and these fixtures
    // carry unrelated warnings anyway, so a status-equality assertion is
    // pass-always: it would hold even if W025 were emitted as an error. The
    // real invariant is that W025 stays a non-escalating, non-repairable
    // warning.
    const data = health(project({ '01-01-SUMMARY.md': '# Summary\n\nCreated: `src/ghost.ts`\n' }));
    assert.strictEqual(w025(data).length, 1, 'precondition: W025 fired');
    assert.ok(
      !(data.errors ?? []).some((e) => e.code === 'W025'),
      `W025 must never appear in errors[]: ${JSON.stringify(data.errors)}`,
    );
    assert.notStrictEqual(data.status, 'broken', 'an advisory must never break health');
    assert.ok(!w025(data)[0].repairable,
      'W025 is diagnostic, not auto-repairable — it must not enter the repair set');
  });

  test('degrades quietly outside a git repo — no crash, no false finding', () => {
    // commits_exist cannot be evaluated without git; absence of git must not
    // manufacture a finding.
    const data = health(project({ '01-01-SUMMARY.md': '# Summary\n\nCreated: `01-01-PLAN.md`\n' }, { git: false }));
    assert.strictEqual(w025(data).length, 0, `no git must mean no finding, got ${JSON.stringify(w025(data))}`);
  });

  test('a phase with no SUMMARY at all produces no finding', () => {
    const data = health(project({}));
    assert.strictEqual(w025(data).length, 0, `absent summaries are not a finding: ${JSON.stringify(w025(data))}`);
  });
});
