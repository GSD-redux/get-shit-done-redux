...
**Scope (#2486 review).** This holds against the capability-based execution gate, not yet against every surface: `quick.md` still gates on the runtime name, so an `orchestrator-worktree` host can be offered a `true` that `/gsd:quick` rejects as fatal, W025 silent because isolation is not `none`. #2728 converts that surface. This PR does not widen the gap — the `≠ none` branch below is unchanged behavior.
