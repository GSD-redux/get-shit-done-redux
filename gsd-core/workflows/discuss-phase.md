...
**If `response_language` is set:** All user-facing output — narration between tool calls included — MUST be presented in `{response_language}`. Technical terms, code, file paths, and subagent prompts stay in English — only user-facing output is translated.
