...
# Executor isolation is a NEGOTIATED CAPABILITY, not a runtime id (ADR-1239
# Codex-binding amendment, #2584). Sibling of `dispatch-should-flatten`, which
# replaced a `RUNTIME === 'codex'` prose rule for the same reason.
# Fail-closed: the verb yields `none` on any unknown/undeclared/error path.
...
# Project-level opt-out still wins on every host.
[ "$USE_WORKTREES" = "false" ] && ISOLATION=none

# A host with NO isolation primitive cannot honor use_worktrees=true — running
# concurrent executors in one checkout races on files, git state, hooks and
# .planning/. Fail closed (unchanged behavior for these hosts, now keyed on the
# declared capability instead of `RUNTIME != claude`).
if [ "$ISOLATION" = "none" ] && [ "$USE_WORKTREES" != "false" ]; then
  echo "FATAL: runtime '$RUNTIME' declares no executor-isolation primitive (dispatch.isolation=none) — worktree isolation would run executor agents unisolated against the main checkout. Set workflow.use_worktrees=false." >&2
  exit 1
fi
...
# Applies to BOTH isolation models: the fork-base divergence is a property of the
# repository, not of who creates the worktree.
...
`ISOLATION` — not `RUNTIME` — selects how the wave fans out. The three values are the only branch points; **never add a `RUNTIME = "codex"` test to this scheduler.** The per-host invocation detail is descriptor data, surfaced by `dispatch-isolation --json` as `harnessFlag` / `exec`:
...
Fail-closed is the invariant: an undeclared, unknown, or unresolvable isolation declaration degrades to `none`, never to an unsafe parallel path. A `harness-worktree` host with no declared flag, and an `orchestrator-worktree` host whose exec descriptor does not resolve, both degrade to `none` rather than dispatching executors that only believe they are isolated.
...
When `USE_WORKTREES` (project-level) is `false`, `ISOLATION` is forced to `none`: all executor agents run unisolated and sequentially on the main working tree instead of in parallel worktrees. The per-plan decision below has no effect when worktrees are project-disabled.

`USE_WORKTREES` and `ISOLATION` are also automatically reset for the duration of a run when `worktree base-check` detects that the orchestrator HEAD has diverged from the worktree fork base (the #683 condition — e.g. an unmerged milestone or feature branch). This check runs for **any** isolated run (`ISOLATION != none`), not only Claude: the fork-base divergence is a property of the repository, so it degrades a GSD-created worktree exactly as it degrades a harness-created one. The auto-degrade prints a one-line warning to stderr and falls through to the sequential path so executors do not hit the exit-42 worktree-branch-check halt. To restore parallel worktree execution, set `worktree.baseRef:"head"` in `.claude/settings.local.json` (or run `gsd-tools worktree set-baseref`) — this makes the fork base track the live HEAD instead of a fixed remote ref. The `worktree-branch-check` exit-42 guard inside each executor remains in place as a backstop.
...
3. **Spawn executor agents:**
   ...
   **Isolation model.** The block below is the **`harness-worktree`** dispatch path (`ISOLATION=harness-worktree`), where the host's own harness creates and binds each worktree. For `ISOLATION=orchestrator-worktree`, skip it and use the orchestrator-managed dispatch further below instead. For `ISOLATION=none`, use sequential mode.
   ...
   **Sequential dispatch for parallel execution (waves with 2+ agents):**
   ...
   Agent(
     ...
     # The host's OWN isolation flag, as declared by its descriptor and surfaced
     # by `gsd_run query dispatch-isolation --json` as `harnessFlag`. On Claude
     # Code that is literally `isolation="worktree"`; on Cursor it is the
     # `--worktree` flag on the agent invocation. Emit the declared token — do
     # NOT hardcode a runtime's flag here.
     {harnessFlag},
     prompt="
       <objective>
   ...
   **Orchestrator-managed worktree dispatch** (`ISOLATION=orchestrator-worktree`):
   ...
   For each runnable plan in the wave, one at a time (`git worktree add` races on `.git/config.lock`):
   ...
   The executor prompt is the same one the harness path uses, minus the harness-specific framing: it still must commit each task atomically, write and commit `SUMMARY.md`, run normal hooks (**no `--no-verify`**), and **not** touch `STATE.md`/`ROADMAP.md`. That last guard needs no new code — `execute-plan` auto-detects worktree mode via the `IS_WORKTREE` (`.git`-is-a-file) primitive, which a GSD-created worktree trips identically to a harness-created one.
