    ...
    description="Execute plan {plan_number} of phase {phase_number}",
     ...
     prompt="
       ...
       <execution_context>
        ...
        From repository root `{phase_dir}/{plan_file}`, read the selected PLAN.md, allowing an optional BOM. Set `PLAN_TDD_CONTEXT=true` for leading frontmatter `type: tdd` (quoted scalar allowed) or a non-fenced task opening tag (may be multiline) with quoted `tdd="true"`; whitespace around `=` is allowed. Reject fenced prose, backticks, and tildes. Conditionally embed `tdd.md`. If the plan/reference is unreadable or literal `${PLAN_TDD_CONTEXT` survives, halt before `Agent()`.
