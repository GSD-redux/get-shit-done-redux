...
**AI integration capability:** If the active `ai-integration` step hook is present, `AI_SPEC_PATH` is empty, and the phase goal contains AI keywords — matched as whole words or phrases, never substrings (`evaluation` does not match `evals`; `storage` does not match `rag`) — (`agent`, `llm`, `rag`, `chatbot`, `embedding`, `langchain`, `llamaindex`, `crewai`, `langgraph`, `openai`, `anthropic`, `vector`, `evals`, `llm eval`, `eval harness`), then:
