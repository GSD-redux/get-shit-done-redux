  ...
  "refactor-trigger": {
    ...
    "config": {
      ...
      "refactor.trigger_strict": {
        "type": "boolean",
        "default": false,
        "description": "Block /gsd-ship while any refactor proposal in the milestone is still untriaged. Off by default and deliberately so: a blocking complexity number is a metric an executor can satisfy by splitting one coherent function into two incoherent ones. The gate therefore clears on the proposal being DISPOSITIONED (gsd-tools refactor accept|decline), never on the score improving. Advisory mode surfaces the same proposal without blocking."
      }
    },
    "commands": [
      {
    ...
    "gates": [
      {
        "point": "ship:pre",
        "check": {
          "query": "refactor.ship-gate"
        },
        "when": "refactor.trigger_strict",
        "blocking": true,
        "onError": "halt"
      }
    ]
