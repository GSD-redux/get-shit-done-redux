...
- Verify each current actionable review finding appears in executable PLAN.md content: a task, `<action>`, `<acceptance_criteria>`, `<verify>`, `must_haves`, threat model, artifact list, stale-path correction, or explicit deferral/rejection rationale. The expected shape for that deferral/rejection rationale — section name, round-scoping, and REVIEWS.md line-anchor format — is the canonical **Review Dispositions Ledger** defined once in `references/planner-reviews.md` (#3806); do not accept or invent a different shape.
